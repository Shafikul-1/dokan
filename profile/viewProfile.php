...
function viewProfile($msg)
{
  ...
  if ($databaseFN->getData("users", "*", null, " uniqueId = '$msg'")) {
    foreach ($databaseFN->getResult() as list("name" => $name, "date" => $date, "email" => $email, "img" => $img)) {
...
?>
      <div class="bg-white md:mx-auto rounded shadow-xl w-full md:w-1/2 overflow-hidden">
        ...
        <div class="px-5 py-2 flex flex-col gap-3 pb-6">
          <div class="h-[90px] shadow-md w-[90px] rounded-full border-4 overflow-hidden -mt-14 border-white"><img src="../upload/<?php echo ($img == null) ? 'user/icon/user-defult-icon.png' : $img ?>" alt="<?php echo ($img == null) ? 'user-defult-icon.png' : $img ?>" class="w-full h-full rounded-full object-center object-cover"></div>
          <div class="">
            <h3 class="text-xl text-slate-900 relative font-bold leading-6 capitalize"><?php echo $name; ?></h3>
          ...
          <div class="flex gap-3 flex-wrap">
            ...
            <span class="rounded-sm bg-blue-100 px-3 py-1 text-xs font-medium text-blue-800 capitalize"><a href="<?php echo $databaseFN->mainUrl . "/whichlist.php" ?>">which list</a></span>
          ...
          <div class="flex gap-2">
            ...
            <button type="button" class="inline-flex w-auto cursor-pointer select-none appearance-none items-center justify-center space-x-1 rounded border border-gray-200 bg-blue-700 px-3 py-2 text-sm font-medium text-white transition hover:border-blue-300 hover:bg-blue-600 active:bg-blue-700 focus:blue-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-300">Add to projects</button>
          </div>
