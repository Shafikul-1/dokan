...
if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
        ...
        $fileObj = new upload();
        $fileObj->uploadFile($_FILES['image'], 'slider');
    ...
    if ($fileUploadComplete) {
        ...
        if ($databaseFN->insertData('slider', $sliderArr)) {
            ...
            echo "<p id='message' class='text-white text-center bg-red-500'>Someting want wrong insert Failed</p>";
        }
    } else {
        echo "<p id='message' class='text-white text-center bg-red-500'>File upload falied</p>";
    }
}
?>
<div class="flex justify-center mt-20 px-8">
    <form action="<?php $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data" class="max-w-2xl">
        <div class="flex flex-wrap border shadow rounded-lg p-3 dark:bg-gray-600">
            ...
            <input type="text" value="homeTop" name="where_slider" hidden id="">
